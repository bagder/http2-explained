...
上一节提到的Stream Identifier将http2连接上传输的每个帧都关联到一个“流”。流指的是同一条http2连接内传输的若干帧组成的一个独立的双向序列。<!-- 这一句翻译的不太好。 -->
...
HTTP是一种无状态的协议。简而言之，这意味着每个请求必须要携带服务器需要的所有细节，而不是让服务器保存住之前请求的元数据。因为http2并没有改变这个范式，所以它不得不以同样方式工作。

这决定了HTTP是一个内容重复性很高的协议。当一个客户端从同一服务器请求了大量资源（例如页面的图片）的时候，就会出现大量几乎完全相同的请求，从而对压缩产生迫切的需求。

每个页面请求的资源数量在增多（如前所述），同时 cookies 的使用和请求的大小也在日渐增长。cookies需要被包含在所有请求中，且他们在多个请求中经常是一模一样的。

HTTP 1.1请求的大小正变得越来越大，有时甚至会大于TCP窗口的初始大小，这会严重拖累发送请求的速度。因为必须先经过一个完整的TCP来回，得到服务端发送的ACK，才能完成整个请求的发送。这也是另一个需要压缩的理由。
...
> “HPACK的设计目标是使信息泄漏更不易发生、使编解码非常快速而代价低廉、使接收方能控制压缩文本的大小、允许代理重新建立索引（如，通过代理在前后端共享状态），以及对哈夫曼编码串的更快速比较”<!-- 这一段需要review -->

## 6.6. 重置 - 后悔药<!-- 这个翻译太别扭了。。-->

HTTP 1.1有一个缺点：当一个含有确切值的Content-Length的HTTP消息被送出之后，你就很难中断它了。当然，通常你可以断开整个TCP链接（但也不总是可以这样），但这样导致的代价就是需要通过三次握手来重新建立一个新的TCP连接。
